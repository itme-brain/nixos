...
{ options.modules.user.gui.wm.hyprland = { enable = mkEnableOption "Enable Hyprland WM"; };
  config = mkIf cfg.enable {
    ...
    home = {
      activation.removeAutogeneratedHyprlandConf = lib.hm.dag.entryAfter [ "linkGeneration" ] ''
        hyprland_conf="${config.home.homeDirectory}/.config/hypr/hyprland.conf"
        if [ -e "$hyprland_conf" ] && ${pkgs.gnugrep}/bin/grep -q "autogenerated = 1" "$hyprland_conf"; then
          ${pkgs.coreutils}/bin/rm -f "$hyprland_conf"
        fi
      '';
